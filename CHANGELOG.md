...
### Added
- **QuickNovel Web Novel Search Integration:** Seamlessly integrated web novel searching and reading into the main search experience without cluttering the manga sources marketplace. Added a switchable media type filter (`📚 Manga` vs `📖 Web Novels`) beneath the search bar. Switching to Web Novels instantly executes concurrent searches across built-in novel providers (`royalroad`, `scribblehub`, `lightnovelworld`, `wuxiaworld`), tags result cards with a purple `Novel` badge, displays provider filter pills for novel platforms, and shows "Popular Web Novels" and "Latest Web Novels" discovery swimlanes in the idle state. Novel extensions are built-in, always enabled, and cleanly isolated from the user-toggleable Sources settings. Backend proxy endpoint (`/manga/proxy/html`) upgraded with `POST` support for paginated novel scraping.
- **Animated Expandable Library Search Bar:** Replaced the static, full-width search input on the Library dashboard (`Dashboard.tsx`) with an animated expandable search bar. Renders as a compact 36×36px icon button when unfocused to keep the dashboard uncluttered; on click or keyboard activation, smoothly expands to 100% width with Framer Motion spring dynamics, autofocusing the input. Automatically collapses back to the compact icon on blur when empty, while preserving expansion whenever an active search query is present. Supports Escape key to clear/collapse and mouse-down protection on the clear button to prevent premature blur collapse.
...
### Removed
- **Mobile Update Available Banner:** Removed the `"Update available — v{version}"` card and update check trigger from the mobile More page (`frontend/src/pages/More.tsx`).
- **Sidebar Desktop App Promotion Badge:** Removed the `"Native Windows App"` / `"Desktop App"` promotional badge card from the desktop sidebar in `frontend/src/App.tsx`.
